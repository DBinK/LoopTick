...
class LoopTick:
    ...
    def __init__(self, first_tick=0.001, auto_report=True):
        """
    ...
    def get_hz(self):
        """查询当前帧率（hz） , 不更新 tick"""
        if self._last_time is None or self.avg_sec == 0:  # 第一次调用返回 1, 避免除零
            return 1
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        print(
            f"第 {i} 次循环耗时: {diff * looptick.NS2MS:.6f} ms, Hz: {hz:.6f}, 平均Hz: {avg_hz:.6f}"
        )
        time.sleep(0.001)
